...
class AppStore extends Component {
  state = {activeItem: tabsList[0].tabId, searchItem: ''}

  getActiveItem = getSearchResult => {
    const {activeItem} = this.state
    const filterItem = getSearchResult.filter(
      eachApp => activeItem === eachApp.category,
    )
  ...
  onChange = tabId => {
    this.setState({activeItem: tabId})
  }
  ...
  render() {
    const {activeItem, searchItem} = this.state
    const getSearchResult = this.getSearch()
    const activeApps = this.getActiveItem(getSearchResult)
        ...
        <ul className="tab-item-container">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabItem={eachTab}
              isActive={eachTab.tabId === activeItem}
              onChange={this.onChange}
            />
          ))}
        </ul>
        <ul className="app-item-container">
          {activeApps.map(eachApp => (
            <AppItem key={eachApp.appId} appItem={eachApp} />
          ))}
        </ul>
      </div>
    )
